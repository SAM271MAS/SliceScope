...
# --- CONFIGURATION & SETUP ---
print("Starting UE1 (Heavy User) - Continuous WebRTC Traffic")

PROXY_HOST = "proxy-service.slicescope.svc.cluster.local"
PROXY_PORT = 3128
ENV_CONTROLLER_URL = "http://env-controller-service.slicescope.svc.cluster.local:9081/state/UE1"
...
while True:
    ...
    # Calculates how much the network has degraded compared to a perfect CQI 15 (80 Mbps)
    scale_factor = cqi_max_mbps / 80.0 
    
    dynamic_base = BASE_TPUT * scale_factor
    dynamic_amp = TREND_AMP_TPUT * scale_factor
    
    # 1. Macro Trend (Uses the dynamically scaled parameters)
    ...
    # 2. Micro Jitter (High momentum for heavy smoothing)
    ...
    telemetry_url = f"{PAYLOAD_SERVER_URL}/100KB.zip?ml_feature=UE1_{app_type}&cqi={current_cqi}&tput={round(actual_mbps, 4)}&jit={round(actual_jitter_ms, 4)}"
