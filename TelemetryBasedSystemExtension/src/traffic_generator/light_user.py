...
# --- CONFIGURATION & SETUP ---
print("Starting UE3 (Light User) - Continuous Web-Server Traffic")

PROXY_HOST = "proxy-service.slicescope.svc.cluster.local"
PROXY_PORT = 3128
ENV_CONTROLLER_URL = "http://env-controller-service.slicescope.svc.cluster.local:9081/state/UE3"
PAYLOAD_SERVER_URL = "http://payload-server-service.slicescope.svc.cluster.local:9080"

# Load the empirical traffic profiles
...
while True:
    ...
    telemetry_url = f"{PAYLOAD_SERVER_URL}/100KB.zip?ml_feature=UE3_{app_type}&cqi={current_cqi}&tput={round(actual_mbps, 4)}&jit={round(actual_jitter_ms, 4)}"
